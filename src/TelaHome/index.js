...
const TelaHome = () => {
  ...
  const [filtroCategoria, setFiltroCategoria] = useState(
    filtrosContext.filtroCategoria,
  );
  ...
  useEffect(() => {
    getRestaurantes();
    getActiveOrder();
  }, [getActiveOrder, getRestaurantes]);

  let categorias = [];

  const pegaCategorias = listaRestaurantes.map(
    (restaurante) => restaurante.category,
  );
  ...
  if (filtrosContext.filtroCategoria !== null) {
    listaRestaurantesFiltrada = listaRestaurantesFiltrada.filter(
      (restaurante) => {
  ...
  return (
    <>
      {!listaRestaurantes || (!busca && listaRestaurantes.length === 0) ? (
        <Loading />
      ) : (
        <Container>
          {!busca ? (
            <Header>
              <HeaderTitulo>Rappi4</HeaderTitulo>
            </Header>
          ) : (
            <Header>
              <HeaderIcone
              ...
              />
              <HeaderTitulo>Busca</HeaderTitulo>
            </Header>
          )}
          ...
          <ListaRestaurantes>
            {busca && filtroBusca.length < 3 && (
              <ResultadoTexto>Busque por nome de restaurantes</ResultadoTexto>
            )}

            {busca &&
              filtroBusca.length > 3 &&
              listaRestaurantesFiltrada.length === 0 && (
                <ResultadoTexto>Nada encontrado :(</ResultadoTexto>
              )}
              ...
              <ContainerFiltro>
                {categorias.map((categoria) => {
                  return (
                    <FiltroCategoria
                      key={categoria}
                      onClick={() => acionaCategoria(categoria)}
                      color={categoria}
            ...
            {listaRestaurantesFiltrada.map((restaurante) => {
              return (
                <CardRestaurante
                  key={restaurante.id}
                  onClick={() => clicaRestaurante(restaurante.id)}
                >
                  <CardImagem
          ...
          {ordem !== null ? (
            <PedidoEmAndamento>
              ...
              <DadosDoPedido>
                <PedidoText>Pedido em andamento</PedidoText>
                <EnderecoRestaurante>
                  {ordem.restaurantName}
                </EnderecoRestaurante>
                <Subtotal>SUBTOTAL R${ordem.totalPrice}</Subtotal>
              </DadosDoPedido>
            </PedidoEmAndamento>
          ) : (
            ''
